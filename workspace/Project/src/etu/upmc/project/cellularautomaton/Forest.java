...
public class Forest extends CellularAutomaton
{
	...
	private static final double DENSITY_TREES 		= 0.40d;
	...
	public void init() 
	{
		for ( int x = 0 ; x < this.width ; x++ )
		{
			...
			{
				if (this.isInState(x, y, CellularAutomaton.EMPTY)) 
				{
	...
	public void step(int x, int y) 
	{
		...
		else if (this.isInState(x, y, CellularAutomaton.FOREST_TREE))
		{
		...
		else if (this.isInState(x, y, CellularAutomaton.FOREST_TREE_BURNING))
		{
			this.informations[x][y]--;
		...
		else if (this.isInState(x, y, CellularAutomaton.FOREST_ASHES))
		{
			this.informations[x][y]++;
