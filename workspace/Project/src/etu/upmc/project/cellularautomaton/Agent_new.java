...
public class Agent extends CellularAutomaton 
{
	...
	private static final double 	PREY_NATALITY_RATE      = 0.10;
	...
	public void step(int x, int y) {
		if (!this.updated[x][y])
		{
			if (this.isInState(x, y, AGENT_PREY) && !checkHunger(x, y, AGENT_PREY))
			{
				...
				/* Reproduction */
				else if (this.nbNeighborsMoore(x, y, AGENT_PREY) > 0 && Math.random() < PREY_NATALITY_RATE)
				{
...
//					int[] coord = this.informations[x][y] > PREY_HUNGER_LIMIT * 0.9 ? this.getPreyHungryCoord(x, y) : this.getRandomCoord(x, y);
			...
			{
				/* Reproduction */
				if (this.nbNeighborsMoore(x, y, AGENT_PREDATOR) > 0 && Math.random() < PREDATOR_NATALITY_RATE)
				{
					int[] coord = this.getRandomCoord(x, y);
					if (!(coord[0] == x && coord[1] == y))
					{
...
//					int[] coord = this.informations[x][y] > PREDATOR_HUNGER_LIMIT * 0.9 ? this.getPredatorHungryCoord(x, y) : this.getRandomCoord(x, y);
		...
		else if (this.isInState(x, y, AGENT_PREY_FLEEING) && !checkHunger(x, y, AGENT_PREY_FLEEING))
		{
			...
			this.moveAgent(x, y, coordPrey[0], coordPrey[1], AGENT_PREY_FLEEING);
			...
			for (int[] coords : predators)
			{
				if (coords[0] != -1)
				{
					...
					this.moveAgent(coords[0], coords[1], newCoords[0], newCoords[1], AGENT_PREDATOR_HUNTING);
					isPredators = true;
				}
			}
		...
		else if (this.isInState(x, y, AGENT_PREY_YOUNGLING))
		{
			...
			if (this.informations[x][y] > PREY_GROWING_TIME)
			{
				System.out.println(this.informations[x][y]);
		...
		else if (this.isInState(x, y, AGENT_PREDATOR_YOUNGLING))
		{
			...
			if (this.informations[x][y] > PREDATOR_GROWING_TIME)
			{
				System.out.println(this.informations[x][y]);
				this.informations[x][y] = 0;
